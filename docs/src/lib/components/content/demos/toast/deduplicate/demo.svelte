...
<Toast.Provider toastManager={manager}>
  <button
    ...
    onclick={bookmarkItem}>Bookmark</button
  >
  ...
  <Toast.Portal>
    ...
    >
      {#each Toast.getToastManager().toasts as toast (toast.id)}
        {@const pulseStyle = toast.updateKey
          ? `animation:${toast.updateKey % 2 === 0 ? 'pulse-even' : 'pulse-odd'} 0.28s ease;`
          : ''}
        <Toast.Root
          {toast}
          style={pulseStyle}
