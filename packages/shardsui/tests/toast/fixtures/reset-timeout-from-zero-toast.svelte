<script lang="ts">
  ...
  function add() {
    toastId = manager.add({ title: 'title', timeout: 0 })
  }
...
<Toast.Provider toastManager={manager}>
  ...
  <button type="button" data-testid="set-timeout" onclick={setTimeoutTo1000}>set timeout</button>
  <button type="button" data-testid="reset-timeout" onclick={resetTimeoutTo1000}
    >reset timeout</button
  >
